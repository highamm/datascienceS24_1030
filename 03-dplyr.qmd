...
ggplot(data = babynames_unique, aes(x = year, y = n_unique)) +
  geom_line()
## this does double count names that are present for each
## sex
```
...
```{r}
babynames_unique_gend <- babynames |> group_by(year, sex) |>
  summarise(n_unique = n())

ggplot(data = babynames_unique_gend,
       aes(x = year, y = n_unique)) +
