...
Locating the source of the tweet from the `statusSource` column and putting that into a new column called `source`.
...
`str_detect()` is making a new variable that is `"quoted"` if `text` starts with a `"` and `"not quoted"` otherwise.
...
tweet_words <- tweets |>
  filter(!str_detect(text, '^"')) |> ## filter out tweets that start with a quote
  mutate(text = str_replace_all(text, "https://t.co/[A-Za-z\\d]+|&amp;", "")) |> ## replace any web link starting with https:.... with nothing
   unnest_tokens(word, text) |>## takes a tweet and converts it to individual words
  anti_join(stop_words, by = c("word" = "word")) |> ## removes all stop words from the word variable
  filter(str_detect(word, "[a-z]")) ## only keep words that have alphabetical letters
...
top_n takes the top 15 logratios

reorder reorders words based on logratio (similar to `fct_reorder()`)

scale_fill_manual changes the colours to colours that we specify

stat = identity is creating this plot where all bars start at 0 and some go left, others go to the right.
