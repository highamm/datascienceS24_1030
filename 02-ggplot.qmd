...
ggplot(data = fitness_full, aes(x = distance)) +
  ...
  facet_wrap(~ stepgoal, ncol = 1)

ggplot(data = fitness_full, aes(x = stepgoal, y = distance)) +
  geom_boxplot()
## easier to see separation here, but you do lose sample size
```
...
ggplot(data = stat113_df, aes(x = GPA, y = Exercise)) +
  geom_line()
## this doesn't make sense because there is more than one 
## Exercise value for each unique GPA
```
...
stat113_df <- read_csv("https://raw.githubusercontent.com/highamm/ds234_quarto/main/data_online/stat113.csv")
glimpse(stat113_df)
## useful if there are a lot of variables in your data
```
...
ggplot(data = stat113_df, aes(x = Wgt, y = Hgt)) +
  ...
  geom_smooth()
## geom_smooth() no longer has the colour aesthetic

ggplot(data = stat113_df, aes(x = Wgt, y = Hgt)) +
  geom_point() +
  geom_smooth(aes(colour = Sport))
## now points are all black but i get three smoothers
```
...
## black and white are not variables so they go outside of an
## aes() function.
```

Your Turn 1.

a. no. "lm" is not a variable so it should not go in aes().

b. this would work!

c. no: colour = "purple" should not be in aes() but colour = groupvar should be in aes().

d. no: colour = groupvar should be in aes().
